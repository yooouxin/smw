...
class IceoryxWriter
{
  public:
        ...
        : m_iceoryx_publisher{nullptr}
    {
        ...
        m_iceoryx_publisher = std::make_unique<iox::popo::UntypedPublisher>(iox_service_desc);
        assert(m_iceoryx_publisher != nullptr);
    }

    SamplePtr<T> loan() noexcept
    {
        ...
        m_iceoryx_publisher->loan(sizeof(T))
    ...
    bool write(SamplePtr<T>&& data) noexcept
    {
        ...
        if constexpr (!IS_FIXED_DATA_TYPE)
        {
            ...
            m_iceoryx_publisher->loan(data_size)
                .and_then([&user_payload](auto& loaned_ptr) { user_payload = loaned_ptr; })
                .or_else([&user_payload](auto& error) { user_payload = nullptr; });
            ...
            m_iceoryx_publisher->publish(user_payload);
        }
        else
        { // fixed size data types --> just publish chunk
            m_iceoryx_publisher->publish(data.get());
  ...
  private:
    std::unique_ptr<iox::popo::UntypedPublisher> m_iceoryx_publisher;
